...
def list_assets():
    ...
    try:
        ...
        # Filtro per ricerca
        if search:
            query = query.filter(Asset.url.ilike(f'%{search}%'))
        
        # Filtro per tipo file
        if file_type:
            query = query.filter(Asset.kind.like(f'{file_type}%'))
        ...
        if sort_by == 'url':
            order_column = Asset.url
        elif sort_by == 'kind':
            order_column = Asset.kind
        else:  # id
        ...
        stats = {
            ...
            'images': db.query(Asset).filter(Asset.kind.like('image%')).count(),
        }
...
def upload_asset():
    ...
    try:
        ...
        for file in files:
            ...
            # Processa immagini se necessario
...
def view_asset(asset_id):
    ...
    try:
        ...
        cards_using_asset = db.query(Card).filter_by(image_id=asset_id).all()
...
def edit_asset(asset_id):
    ...
    try:
        ...
        # POST: aggiorna asset
        new_alt = request.form.get('alt', '').strip()
        
        if not new_alt:
            flash('Il testo alternativo è obbligatorio', 'error')
            return redirect(request.url)
        
        # Controlla se il nuovo nome è già in uso
        if new_alt != asset.alt:
            existing = db.query(Asset).filter_by(
                alt=new_alt
            ).filter(Asset.id != asset_id).first()
            
            if existing:
                flash('Testo alternativo già in uso', 'error')
                return redirect(request.url)
...
def delete_asset(asset_id):
    ...
    try:
        ...
        cards_using = db.query(Card).filter_by(image_id=asset_id).count()
        ...
        # Elimina file fisico
        file_path = os.path.join(current_app.static_folder, 'media', asset.url)
        ...
        flash(f'Asset "{asset_alt}" eliminato con successo!', 'success')
        return redirect(url_for('assets.list_assets'))
    ...
    try:
        ...
        return jsonify({
            ...
            'asset': {
                ...
                'url': new_asset.url,
                'alt': new_asset.alt,
                'full_url': url_for('static', filename=f'media/{new_asset.url}')
            }
        })
